...
const CARRY_DEPTH = 1_000_002; // предмет в лапах рисуется по центру персонажа, поэтому он
                               // должен быть выше него (DEPTH.player в WorldScene) — иначе
                               // спрятался бы за спрайтом

// Предмет с alwaysOnTop (чашка кофе) рисуется поверх всего мира, включая overlay
// локации (DEPTH.doorOverlay = 2_000_000) — и в лапах, и стоя на столе.
export const ITEM_TOP_DEPTH = 2_500_000;
...
export class PhysicsItem {
  readonly id: string;
  readonly typeKey: string; // ключ в ITEM_TYPES ("tennis", "coffee", ...) — им обмениваемся с сервером
  ...
  expiresAt = 0;          // время (scene.time.now), когда предмет исчезает; 0 — вечный (чашка кофе)

  constructor(scene: Phaser.Scene, def: PlacedItemDef) {
    this.id = def.id;
    this.typeKey = def.type;
  ...
  // Взят в лапы: физика замирает, предмет рисуется ровно по центру спрайта игрока.
  carryTo(x: number, y: number): void {
    ...
    this.sprite.setDepth(this.type.alwaysOnTop ? ITEM_TOP_DEPTH : CARRY_DEPTH);
  ...
  update(delta: number): void {
    ...
    const depth = this.type.alwaysOnTop ? ITEM_TOP_DEPTH : this.base.y;
